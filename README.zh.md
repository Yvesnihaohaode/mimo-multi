...
A：请求里带了图片，但你用的模型（比如 `mimo-v2.5-pro`、`mimo-v2-flash`）**不支持图像输入**。MiMo 系列里只有 `mimo-v2-omni` 支持图片 / 音频 / 视频。

mimo2codex 现在会**自动丢弃非 omni 模型上的图像部分**并加一条短文本提示给模型，请求不会再 404。日志里会有：

```
WARN dropped 1 image part(s) — model "mimo-v2.5-pro" does not support image input (only mimo-v2-omni does)
```

如果你**确实**要用视觉能力，把 model 改成 `mimo-v2-omni`：
